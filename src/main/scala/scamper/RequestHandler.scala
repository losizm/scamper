...
trait RequestHandler {
   ...
   *
   * If the handler fulfills the request, then a corresponding response is
   * returned. Otherwise, the handler passes the request, as is or with
   * modifications, to the next handler.
   */
